...
// Add these methods to your paymentController.js

/**
 * @desc    Create wallet top-up order
...
exports.createWalletOrder = async (req, res) => {
  try {
    ...
    errorResponse(res, 500, 'Error creating wallet order', error.message);
  }
};

// @route POST /api/payment/verify-wallet-payment
exports.verifyWalletPayment = async (req, res) => {
  try {
    ...
    successResponse(res, 200, 'Wallet topped up', { newBalance: profile.walletBalance });
  } catch (error) {
    errorResponse(res, 500, 'Verification failed', error.message);
  }
};